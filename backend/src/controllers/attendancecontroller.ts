...
// AttendanceController: operations for attendance records
export interface AttendanceController {
  insertIntoAttendance: (status: string, date: string, studentnumber: string, lectureid: string) => Promise<unknown>;
  checkAndInsertStatusNotPresentAttendance: (date: string, studentnumbers: string[], lectureid: string) => Promise<unknown>;
  getLecturesAndAttendancesByCourseId: (courseid: string) => Promise<unknown>;
  updateAttendanceStatus: (attendanceid: number, status: number) => Promise<unknown>;
  deleteAttendance: (studentnumber: string, lectureid: string) => Promise<unknown>;
  markStudentAsNotPresentInPastLectures: (studentnumber: string | number, courseid: number) => Promise<void>;
}

const attendanceController: AttendanceController = {
  // Insert one attendance row
  async insertIntoAttendance(status: string, date: string, studentnumber: string, lectureid: string): Promise<unknown> {
    try {
      ...
      const usercourseResult = await usercoursesModel.getUserCourseId(studentnumber, courseId);
      if (!Array.isArray(usercourseResult) || usercourseResult.length === 0) {
        ...
        throw new Error(`Usercourse not found for the studentnumber: ${studentnumber}`);
      }
      ...
      if ('usercourseid' in usercourseResult[0]) {
        ...
        const attendanceResultCheck = await attendanceModel.checkAttendance(usercourseid, Number(lectureid));
        if (!attendanceResultCheck || attendanceResultCheck.length > 0) {
          ...
          throw new Error(`Attendance already exists for the usercourseid: ${usercourseid}`);
        }

        const insertResult = await attendanceModel.insertAttendance(Number(status), date, usercourseid, lectureid);
        ...
        const attendanceResult = await attendanceModel.getAttendanceById(insertResult[0].insertId);
  ...
  // Insert "not present" for students missing an attendance row
  async checkAndInsertStatusNotPresentAttendance(date: string, studentnumbers: string[], lectureid: string): Promise<void> {
    try {
      for (const studentnumber of studentnumbers) {
        ...
        const usercourseResult = (await usercoursesModel.getUserCourseId(studentnumber, courseId)) as RowDataPacket[];
        ...
        const usercourseid = usercourseResult[0].usercourseid;
        const attendanceResult = await attendanceModel.getAttendanceByUserCourseIdDateLectureId(usercourseid, lectureid);

        if (attendanceResult.length === 0) {
          console.log("row 230, attendanceController.ts, attendanceResult is empty: ",attendanceResult);
          await attendanceModel.insertAttendance(0, date, usercourseid, lectureid);
        }
      }
  ...
  async deleteAttendance(studentnumber: string, lectureid: string): Promise<boolean> {
    try {
      const courseId = await lectureModel.getCourseIDByLectureID(lectureid);
      ...
      if ('usercourseid' in usercourseResult[0]) {
        ...
        const deleteResult = await attendanceModel.deleteAttendance(usercourseid, Number(lectureid));
  ...
  async markStudentAsNotPresentInPastLectures(studentnumber: string | number, courseid: number): Promise<void> {
    try {
      ...
      const usercourseResult = await usercoursesModel.getUserCourseId(studentNumberString, courseId);
      if (!Array.isArray(usercourseResult) || usercourseResult.length === 0) {
        console.log("row 368, attendanceController.ts, usercourseResult is empty: ",usercourseResult);
        throw new Error(`Usercourse not found for the studentnumber: ${studentNumberString}`);
      }
      ...
      if ('usercourseid' in usercourseResult[0]) {
        ...
        for (const lecture of pastLectures) {
          ...
          const attendanceResultCheck = await attendanceModel.checkAttendance(usercourseid, lecture.lectureid);
          if (!attendanceResultCheck || attendanceResultCheck.length > 0) {
            console.log("row 389, attendanceController.ts, attendanceResultCheck is empty: ",attendanceResultCheck);
            continue;
          }
          await attendanceModel.insertAttendance(0, lecture.start_date, usercourseid, lecture.lectureid.toString());
        }
      } else {
