...
const AdminLectureDetail: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!courseId || !lectureId) {
        toast.error(t('admin:TeacherLectures.error.missingIds'));
        const err = new Error(
      ...
      if (!response || response.length === 0) {
        toast.info(t('admin:TeacherLectures.info.noData'));
      } else {
        setData(response);
      }
    } catch (err) {
      ...
      toast.error(
        error.details ||
          error.message ||
          t('admin:TeacherLectures.error.fetchAttendances'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (attendanceId: number) => {
    ...
    try {
      ...
      toast.success(t('admin:TeacherLectures.success.deleteAttendance'));

      // Refresh the data after deleting an attendance
      fetchData();
    } catch (error) {
      toast.error(t('admin:TeacherLectures.error.deleteAttendance'));
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    return (
      <div className='p-6 m-4 text-center rounded-lg bg-metropolia-support-red-dark/10'>
        ...
        <p className='mt-2 text-metropolia-main-grey'>
          {error.message || t('admin:TeacherLectures.error.unknown')}
        </p>
        <button
          onClick={fetchData}
    ...
    <div className='p-6 m-4 overflow-hidden rounded-lg shadow-md bg-metropolia-support-white'>
      ...
      <div className='mb-6 border-b border-gray-200 pb-4'>
        <Typography
          ...
          {t('admin:TeacherLectures.detail.title')}
        </Typography>
        ...
        <div className='flex justify-between mt-4'>
          <span className='text-sm text-metropolia-main-grey'>
            {data
              ? t('admin:TeacherLectures.detail.totalAttendances', {
                  count: data.length,
                })
              : t('admin:TeacherLectures.detail.noData')}
          </span>
          <button
            onClick={fetchData}
          ...
          <TableHead>
            <TableRow className='bg-gradient-to-r from-metropolia-main-orange to-metropolia-secondary-orange'>
              <TableCell className='font-heading text-metropolia-support-white'>
                {t('admin:ui.attendanceId')}
              </TableCell>
              <TableCell className='font-heading text-metropolia-support-white'>
                {t('admin:ui.userCourseId')}
              </TableCell>
              <TableCell className='font-heading text-metropolia-support-white'>
                {t('admin:ui.name')}
              </TableCell>
            ...
            {data && data.length > 0 ? (
                    ...
                    <div className='flex items-center'>
                      ...
                      {duplicateUserCourseIds.has(item.usercourseid) && (
                        <span className='px-2 py-0.5 ml-2 text-xs rounded bg-metropolia-support-yellow/20 text-metropolia-main-grey'>
                          {t('admin:TeacherLectures.detail.duplicate')}
                        </span>
                      )}
                    </div>
                  </TableCell>
              ...
              <TableRow>
                <TableCell
                  ...
                  {t('admin:TeacherLectures.detail.noAttendanceData')}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Dialog
        ...
        <DialogTitle id='alert-dialog-title' className='font-heading'>
          {t('admin:TeacherLectures.delete.title')}
        </DialogTitle>
        <DialogContent>
          <DialogContentText id='alert-dialog-description'>
            {t('admin:TeacherLectures.delete.deleteAttendanceConfirm')}
          </DialogContentText>
        </DialogContent>
