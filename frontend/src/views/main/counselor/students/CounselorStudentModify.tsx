...
/**
 * CounselorStudentModify component.
 ...
 *
 * @returns {JSX.Element} The rendered CounselorStudentModify component.
 */
const CounselorStudentModify: React.FC = () => {
  /**
  ...
  useEffect(() => {
    if (user) {
      ...
      const ModifyUserData = async () => {
        try {
          const modifyUser = await apiHooks.fetchUserByIdEdit(
            Number(userid),
            token,
          );
  ...
  const handleSave = async (editedUser: User) => {
    ...
    try {
      const response = await apiHooks.updateUserEdit(token, editedUser);
...
export default CounselorStudentModify;
